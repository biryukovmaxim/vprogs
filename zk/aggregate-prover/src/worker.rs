        ...
        >,
{
    ...
    async fn run(mut self) {
        loop {
            ...
            // Nothing ready: park until a new command arrives, a settlement advances, or shutdown.
            // `pop` and the settlement check re-run at the top of the next iteration, so a signal
            // between here and the drain is not missed. Shutdown is checked first so a teardown
            // request is never starved by a busy watch and the worker's join never hangs. The
            // settlement wake is low-priority (after the inbox): a pending settlement
            // re-aggregation is cheap and can wait behind real proving work.
